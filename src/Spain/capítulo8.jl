function BdE_be08(ruta::String, from::Date) 

    cd(ruta)
    pwd()
    #Descarga de ficheros desde la página del Banco de España
    file = HTTP.get("https://www.bde.es/webbde/es/estadis/infoest/series/be08.zip").body

    r = ZipFile.Reader(IOBuffer(file));

    for f in r.files
        println("Filename: $(f.name)")
        io = open("be08/"*f.name,"w") #Creación de fichero con nombre igual al del ZIP
        write(io,read(f))
        close(io)
    end
    close(r)
    ...
    for file in readdir("be08")

        #Lectura fichero transformando Latin1 a UTF8      
        tempdf = CSV.read(open("be08/"*file, enc"Latin1"), header=3, datarow=5, delim=',',quotechar='"',missingstring="_")
