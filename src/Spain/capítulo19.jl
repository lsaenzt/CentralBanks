function BdE_be19(ruta::String, from::Date) 

    cd(ruta)
    pwd()
    #Descarga de ficheros desde la página del Banco de España
    file = HTTP.get("https://www.bde.es/webbde/es/estadis/infoest/series/be19.zip").body

    r = ZipFile.Reader(IOBuffer(file));

    for f in r.files
        println("Filename: $(f.name)")
        io = open("be19/"*f.name,"w") #Creación de fichero con nombre igual al del ZIP
        write(io,read(f))
        close(io)
    end
    close(r)
    ...
    #Lectura datos ficheros BdE. La carpeta debe estar en una carpeta llamada "be08"
    for file in readdir("be19")

        #Lectura fichero transformando Latin1 a UTF8      
        tempdf = CSV.File(open("be19/"*file, enc"Latin1"), header=4, datarow=7, footerskip=2, delim=',',quotechar='"',missingstring="_") |> DataFrame
